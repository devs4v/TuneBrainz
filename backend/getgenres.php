...
error_reporting(0);	//remove all warnings
// Search in books
...
foreach($music as $code){
	...
	if(isset($response['feed']['entry'])){
		foreach($response['feed']['entry'] as $item){
			...
			$musicdata['track'] = $item['im:name']['label'];
			$musicdata['cover'] = $item['im:image']['2']['label'];
			$musicdata['album'] = $item['im:collection']['im:name']['label'];
			$musicdata['albumlink'] = $item['im:collection']['link']['attributes']['href'];
			$musicdata['duration'] = $item['link'][1]['im:duration']['label'];
			$musicdata['preview'] = $item['link'][1]['attributes']['href'];
			$musicdata['producer'] = $item['rights']['label'];
			$musicdata['artist'] = $item['im:artist']['label'];
			$musicdata['genre'] = $item['category']['attributes']['label'];
			$musicdata['releasedate'] = $item['im:releaseDate']['attributes']['label'];
			$musicdata['link'] = $item['id']['label'];
...
foreach($books as $code){
	...
	if(isset($response['feed']['entry'])){
		foreach($response['feed']['entry'] as $item){
			...
			$bookdata = array();
			$bookdata['name'] = $item['im:name']['label'];
			$bookdata['image'] = $item['im:image']['2']['label'];
			$bookdata['summary'] = $item['summary']['label'];
			$bookdata['author'] = $item['im:artist']['label'];
			$bookdata['publisher'] = $item['im:publisher']['label'];
			$bookdata['genre'] = $item['category']['attributes']['label'];
			$bookdata['releasedate'] = $item['im:releaseDate']['attributes']['label'];
			$bookdata['link'] = $item['id']['label'];
